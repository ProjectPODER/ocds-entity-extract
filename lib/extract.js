    ...
    if(compiledRelease.hasOwnProperty('parties')) {
        ...
        parties.map( (party) => {
            ...
            switch(party.details.type) {
                case 'institution':
                    handleInstitution(compiledRelease, party, entities, contractItems, metadata);
                    break;
                default:
                    let isCompany = null;
                    ...
                    if(isCompany) handleCompany(compiledRelease, party, entities, contractItems, metadata);
                    else handlePerson(compiledRelease, party, entities, contractItems, metadata);
                    break;
            }
        } );
    }
            ...
            if(contract.hasOwnProperty('items') && contract.items.length > 0) {
                contract.items.map( item => {
                    ...
                    if(prodID && cbmeiProd) {
                        ...
                        product.contract_amount += item.valueMxIMSS;
                        product.contract_quantity += item.quantity;
                        if(item.hasOwnProperty('unit') && item.unit.hasOwnProperty('value') && item.unit.value.hasOwnProperty('amountOverpriceMxIMSS') && item.unit.value.amountOverpriceMxIMSS != 0) {
                            product.amount_over += parseFloat(item.unit.value.amountOverpriceMxIMSS);
                            if(item.unit.value.valueAverageMxIMSS != 0)
                                product.quantity_lost += parseFloat(item.unit.value.amountOverpriceMxIMSS / item.unit.value.valueAverageMxIMSS);
                        }
...
function handlePerson(compiledRelease, party, entities, items, metadata) {
    let person = null;
...
function handleCompany(compiledRelease, party, entities, items, metadata) {
    let company = null;
...
function handleInstitution(compiledRelease, party, entities, items, metadata) {
    ...
    if(party.hasOwnProperty('memberOf') && party.memberOf[0].id != '') {
    ...
    let amountName = '';
    if(items.length > 0) {
        summary = getContractItemSummary(compiledRelease, items);
        countName = 'purchase_count';
        amountName = 'purchase_amount';
    }
    else {
        summary = getBuyerContractSummary(institutionID, compiledRelease);
    ...
    if(countryID) {
        entities.states[countryID][countName] += summary[0];
        entities.states[countryID][amountName] += parseFloat(summary[1]);
    }
    if(stateID) {
        entities.states[stateID][countName] += summary[0];
        entities.states[stateID][amountName] += parseFloat(summary[1]);
    }
    if(municipalityID) {
        entities.states[municipalityID][countName] += summary[0];
        entities.states[municipalityID][amountName] += parseFloat(summary[1]);
    }
}
...
function getCountryName(code) {
    switch(code) {
        ...
        case 'US': return 'Estados Unidos de América';
        case 'UY': return 'Uruguay';
        case 'VE': return 'Venezuela';
        case 'VI': return 'Islas Vírgenes';
