...
function handleInstitution(compiledRelease, releases, party, entities, items, metadata) {
    ...
    if(!instIndex) {
        ...
        // Check in suppliers if this institution has been seen before as a supplier
        let companyIndex = findObjectInCollection(institutionID, entities.companies);
        if(companyIndex) {
            institution.classification = ['institution'];
            institution.contract_count = entities.companies[institutionID].contract_count;
            institution.contract_amount = entities.companies[institutionID].contract_amount;
            institution.identifiers = entities.companies[institutionID].identifiers;
            delete entities.companies[institutionID];
        }
