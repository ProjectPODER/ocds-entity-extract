    ...
    if(govLevel != null) {
        ...
        switch(govLevel) {
            case 'city':
                ...
                if(!stateIndex) {
                    // Crear el estado
                    state = createState('state', stateID, party.address.region, countryID, metadata);
                ...
                if(!municipalityIndex) {
                    // Crear la municipalidad
                    municipality = createState('municipality', municipalityID, party.address.locality, stateID, metadata);
...
function createState(type, id, name, parent_id = null, metadata) {
    let state = {
        id: id,
        name: name,
        classification: type,
    ...
    if(parent_id)  Object.assign(state, { parent_id: parent_id });
    else           Object.assign(state, { parent_id: 'mx' });
